...
\begin{frame}{Laplace transform approach to zero-input response $\mf{x}\ct{t}$}
\begin{itemize}
...
\end{itemize}
\demoex{2}{Find $\mf{x}\ct{t}$ for $t \geq 0$: $\dot{\mf{x}}\ct{t} = \bmx 1 & 2 \\ 0 & 2\emx \mf{x}\ct{t}$.}

\end{frame}


\begin{frame}{Properties of $e^{t\mf{A}}$}
\begin{itemize}
...
\cols{}{
    ...
    \col{0.5\textwidth}{
        \begin{enumerate}
            ...
            \item \demoexc{3}{black}{Describe the state equation of a mass $M$ in free space. What are its modes? 
            }
        \end{enumerate}
    }
}
...
\begin{frame}{Complete solution for $\mf{x}\ct{t}$ and $\mf{y}\ct{t}$}
\begin{footnotesize}
\begin{itemize}
...
\end{itemize}
\end{footnotesize}
\demoexc{2}{gray}{\footnotesize Find the impulse response matrix for $\mf{A} = \bmx 1 & 2\\0 & 1\emx$, $\mf{B} = \bmx 1 & -0.5\\1 & 1\emx$, $\mf{C} = \bmx 1 & 0\emx$, and $\mf{D} = 0$.}

\demoexc{3}{gray}{{\footnotesize Find the expression for $\mf{y}\ct{t} = \bmx v_{C_1}\ct{t} & v_{R_2}\ct{t}\emx^T$ for the following system, such that $v_{C_1}\ct{0^-} = 1V$, $v_{C_2}\ct{0^-} = -0.5V$, $u_1\ct{t} = 1\ct{t}V$, and $R=1k\Omega, C=1mF$.
\vspace{-0.3cm}
\begin{center}
...
\begin{frame}{Solution for discrete-time LTI system}
\begin{itemize}
...
\cols{t}{
    \col{0.5\textwidth}{
        ...
        \[ \tilde{\mf{x}}\dt{k} = \mf{\Lambda}^k\tilde{\mf{x}}\dt{0} = \bmx 
         ...
         & & \ddots & \\
         & & &  \lambda_n^k
        \emx \tilde{\mf{x}}\dt{0} \]\vspace{-0.4cm}

        An arbitrary initial state $\mf{x}\dt{0} = \sum_{i=1}^{n}\alpha_i\mf{v}_i$ evolves as follows,\vspace{-0.4cm}
        \[ \mf{x}\dt{k} = \alpha_1 \lambda_1^k\mf{v}_1 + \alpha_2 \lambda_2^k\mf{v}_2 + \ldots + \alpha_n \lambda_n^k\mf{v}_n\]
        \vspace{0.1cm}

        \textbf{When $\mf{A}$ is not diagonalizable}, $\mf{A} = \mf{V}\mf{J}\mf{V}^{-1}$\vspace{-0.3cm}
