...
\demoex{2}{Derive and solve the differential equation representing the voltage-current relationship.}
\begin{columns}
...
\draw
  (ref_gnd) to[american voltage source=$v\lp t\rp$] ++(0,2)
            ...
            to[R=\(R\)] ++(0,-2) 
  -- (ref_gnd);
  \node[xshift=1.0cm, below, blue] at (0,0) {$u\lp t\rp = v\lp t\rp, y\lp t\rp = v_R\lp t\rp$};
\end{tikzpicture}
}
\end{column}
...
\end{columns}
\vspace{-0.25cm}
\demoex{5}{What is the output for each of the three systems when $v\lp t\rp = v_0 \times 1\lp t\rp$?}
\end{frame}
...
\begin{frame}{Transfer function}
\demoex{1}{Derive the transfer function and impulse response for the following systems.}
\begin{columns}
\begin{column}{0.275\textwidth}
\demoexc{2}{black}{
\begin{tikzpicture}[scale=0.75, transform shape]
